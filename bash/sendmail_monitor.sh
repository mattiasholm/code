...
if ps -e | grep -q sendmail-mta; then
   result="true"
else
   result="false"
fi
...
"false")
   ...
   if ps -e | grep -q sendmail-mta; then
      echo "$(date)\tRECOVERY: Service sendmail successfully started again" | sudo tee -a /var/log/sendmail.log
   else
      echo "$(date)\tCRITICAL: Failed to recover service sendmail. Reporting this to driftlarm@donator.se" | sudo tee -a /var/log/sendmail.log
      (
         sleep 1
