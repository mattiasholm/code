...
import { name, strip, cidrSubnet } from './functions.ts';
...
let log: operationalinsights.Workspace | undefined;
...
const pips: Record<string, network.PublicIPAddress> = {};
const cnames: Record<string, privatedns.PrivateRecordSet> = {};
...
config.pipLabels.forEach((label, i) => {
    ...
    const cname = new privatedns.PrivateRecordSet(`cname_${label}`, {
        ...
        recordType: 'CNAME',
        cnameRecord: {
            cname: pip.dnsSettings.apply(dnsSettings => dnsSettings!.fqdn!),
        },
    });
...
const sts: storage.StorageAccount[] = [];
...
Object.entries(config.roles).forEach(([key, value]: [string, config.Role]) => {
