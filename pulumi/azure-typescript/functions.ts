import { DefaultAzureCredential } from '@azure/identity';
import { AuthorizationManagementClient } from '@azure/arm-authorization';
import { authorization } from '@pulumi/azure-native';
import * as config from './config.ts';

export async function getRoleId(roleName: string): Promise<string> {
    const credential = new DefaultAzureCredential();
    const subscriptionId = (await authorization.getClientConfig()).subscriptionId;
    const client = new AuthorizationManagementClient(credential, subscriptionId);

    for await (const role of client.roleDefinitions.list(`/subscriptions/${subscriptionId}`)) {
        if (role.roleName === roleName) {
            return role.id!;
        }
    }

    throw new Error(`Role '${roleName}' not found.`);
}

export function name(resourceType: string, instance: number = 1): string {
    return `${resourceType}-${config.prefix}-${String(instance).padStart(2, '0')}`;
}

export function strip(name: string): string {
    return name.replace(/-/g, '');
}

export function cidrSubnet(network: string, newCidr: number, subnetIndex: number): string {
    const ipToInteger = (ip: string) => ip.split('.').reduce((acc, octet) => (acc << 8) + +octet, 0);
    const integerToIp = (int: number) => `${(int >>> 24) & 255}.${(int >>> 16) & 255}.${(int >>> 8) & 255}.${int & 255}`;

    const baseInteger = ipToInteger(network.split('/')[0]);
    const subnetSize = 2 ** (32 - newCidr);
    const subnetInteger = baseInteger + subnetSize * subnetIndex;

    return `${integerToIp(subnetInteger)}/${newCidr}`;
}
