...
resource "azurerm_key_vault_access_policy" "accesspolicy" {
  ...
  # tenant_id          = data.azurerm_client_config.current.tenant_id # OBS: Går det hämta ut tenantId specifikt för managedIdentity även i ARM/Bicep??? Isåfall borde jag såklart ändra och göra på samma sätt där, istället för att gå på subscription().tenantId!
